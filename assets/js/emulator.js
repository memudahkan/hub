(() => {
    ...
    const searchableItems = Array.from(document.querySelectorAll('.resource-list li')).map((item) => {
      ...
      const searchableText = [item.textContent, subtitle ? subtitle.textContent : '', sectionTitle ? sectionTitle.textContent : ''].join(' ').toLowerCase();
      return { item, group, section, searchableText };
    });
    ...
    const runSearch = (rawQuery) => {
      ...
      let matchCount = 0;
      if (heroSection) heroSection.hidden = !!query;
      if (featuredSection) featuredSection.hidden = !!query;
      searchableItems.forEach(({ item, searchableText }) => {
        ...
        if (matched) {
          matchCount++;
          if (query) highlightInElement(item, query);
        }
      });
      document.querySelectorAll('.resource-group').forEach((group) => {
        const visibleItems = Array.from(group.querySelectorAll('.resource-list li')).some((item) => !item.hidden);
        group.hidden = !visibleItems;
      });
      document.querySelectorAll('.section').forEach((section) => {
        if (featuredSection && section === featuredSection) return;
        const hasLists = !!section.querySelector('.resource-list');
        if (!hasLists) {
          section.hidden = false;
          return;
        }
        const visibleItems = Array.from(section.querySelectorAll('.resource-list li')).some((item) => !item.hidden);
    ...
    const syncSearchInputs = (source, value) => {
      if (source !== desktopSearchInput && desktopSearchInput) desktopSearchInput.value = value;
      if (source !== mobileSearchInput && mobileSearchInput) mobileSearchInput.value = value;
    };
